...
<div id="answer-state-<?php echo $a ?>" class="answer-state">
    <?php if ($get_answers['answer_state'] == 1 && isset($_SESSION['user_id']) && $_SESSION['user_id'] == $data['question_by']) { ?>
        <form class="answer-state-form" name="answerState"
              ...
              method="post">
            <button type="submit" name="state" value="2" onchange="checkState(<?php echo $a ?>); this.form.submit()">
                Accept Answer
            </button>&nbsp;
            <button type="submit" name="state" value="0" onchange="checkState(<?php echo $a ?>); this.form.submit()">
                Refuse Answer
            </button>
        </form>
    <?php } elseif ($get_answers['answer_state'] == 2) { ?>
        <span>Answer has been accepted.</span>
    <?php } elseif ($get_answers['answer_state'] == 0) { ?>
