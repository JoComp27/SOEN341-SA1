...
$result = $db->query("SELECT count(1) from answers_userlikes where user_id='$user_id'");
...
if ($isPresent == 0) {
    ...
    $db->query("INSERT INTO answers_userlikes VALUES ('$id','$userId')");
} else {
    $valueAfterUpdate = $row['answers_upvotes'] - 1;
    $db->query("update answers set answers_upvotes='$valueAfterUpdate' where answers_id='$id'");
    $db->query("DELETE FROM answers_userlikes VALUES ('$id','$userId')");
}
?>