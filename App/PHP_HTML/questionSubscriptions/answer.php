...
<head>
    ...
    <script>
		...
		function qstSubscription(id1, id2) {
			if($bool3 = null){
				<?php
					$sub_id = $_SESSION['user_id'];
					$qst_id = $_GET['id'];
					$select_query = "insert into question_subscription (subscriber_id, question_id) values ('$sub_id', '$qst_id')";
...
<ul class="list-group">
    <li class="list-group-item"><b> <?php echo
            ...
            <a href="profile.php?id=<?php 
				...
				echo $id;
			?>"><?php echo $data['question_by_user']; ?></a></b></li>
    <li>
        <?php
	...
	<li>
		<button type="Subscribe" name="subscribe" value="Subscribe to this question!" class="btn btn-success" onclick="qstSubscription($usr_id, $qus_id)">Subscribe to this question!</button>
		<?php
		if($bool2 = null){
				$title_question = $data['question_title'];
				$url = "<a href=\'answer.php?id=$qus_id\'><h4>answer.php?id=$qus_id</h4></a>";
				$answers_by_user = $_SESSION['user_name'];
				$select_query = "insert into notification (notification_title, notification_date, notification_content) values('$answers_by_user subscribed to your question',NOW(), 'You received a new subscription from $answers_by_user for question: $title_question $url')";
				$sql = mysqli_query($db, $select_query);
				$latest_local_notification_id = mysqli_fetch_assoc(mysqli_query($db, "SELECT LAST_INSERT_ID() as 'result'"))['result'];
				$question_user_id = $data['question_by'];
				$select_query = "insert into notification_user (notification_id, user_id) values ('$latest_local_notification_id', '$question_user_id')";
				$sql = mysqli_query($db, $select_query);
				$bool2 = true;
		}
		else{
...
<ul class="list-group">
	...
	if($bool = null){
			...
			$url = "<a href=\'answer.php?id=$qus_id\'><h4>answer.php?id=$qus_id</h4></a>";
			$answers_by_user = $_SESSION['user_name'];
			$select_query = "insert into notification (notification_title, notification_date, notification_content) values('$answers_by_user answered a question you were subscribed to',NOW(), 'A reply from $answers_by_user for subscribed question: $title_question $url was received')";
			$slct_query = "select * from question_subscription WHERE question_id='$qus_id'";
			$sql2 = mysqli_query($db, $slct_query);
			$get_data = mysqli_fetch_assoc($sql2);
			$sql = mysqli_query($db, $select_query);
			$latest_local_notification_id = mysqli_fetch_assoc(mysqli_query($db, "SELECT LAST_INSERT_ID() as 'result'"))['result'];
			$question_user_id = $get_data['subscriber_id'];
			$select_query = "insert into notification_user (notification_id, user_id) values ('$latest_local_notification_id', '$question_user_id')";
			$sql = mysqli_query($db, $select_query);
			$bool= true;
	}
